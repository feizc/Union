...
def parse_args():
    ...
    parser.add_argument(
        "--tensorboard_path", type=str, default="/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtcv/feizhengcong/clip/union/tensorboard/ema_multiview",
    )
...
def main(): 
    ...
    train_loader = get_cc3m_dataset(args, preprocess_train, is_train=True, tokenizer=tokenizer, multi_view=True) 
    ...
    for epoch in range(32): 
        ...
        for i, batch in enumerate(train_loader):  
            ...
            images, aug_images, texts = batch 
            images = images.to(device=device)
            aug_images = aug_images.to(device=device)
            texts = texts.to(device=device)
            optimizer.zero_grad()

            with autocast():
                image_features, text_features, logit_scale = model(images, texts, combine=False) 
                image_text_features, _ = ema_model(aug_images, texts, combine=True) 
